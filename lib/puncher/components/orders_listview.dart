...
import 'package:future_hub/common/shared/widgets/generic_sticky_header_delegate.dart';
import 'package:future_hub/common/shared/widgets/sliver_infinite_list_view.dart';

class PuncherOrdersListView extends StatefulWidget {
  final ScrollController? controller;
  const PuncherOrdersListView({super.key, this.controller});
...
class _PuncherOrdersListViewState extends State<PuncherOrdersListView> {
  Future<void> _onLoadMore() async {
    ...
    if (authState is AuthSignedIn && authState.user.puncherTypes!.contains('Fuel')) {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<ServiceProviderOrdersCubit, ServiceProviderOrderStates>(
      builder: (context, state) {
        final authState = context.read<AuthCubit>().state;
        final isFuelUser =
            authState is AuthSignedIn && authState.user.puncherTypes!.contains('Fuel');
        if (state is ServiceProviderOrdersLoadingState && state.isFirstFetch ||
            state is ServiceProviderOrdersServicesLoadingState && state.isFirstFetch) {
          return const SliverFillRemaining(
            child: Center(child: PaginatorLoadingIndicator()),
          );
        }
        List<Datum> orders = [];
        bool canLoadMore = context.read<ServiceProviderOrdersCubit>().canLoadMoreFuel;
        ...
        final filteredOrders = orders.where((order) => order.status == 1).toList();
        ...
        return SliverMainAxisGroup(
          slivers: [
            SliverPersistentHeader(
              pinned: true,
              delegate: GenericStickyHeaderDelegate(
                height: 80,
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      Text(
                        " (${t.count_orders(state is ServiceProviderOrdersLoadedState ? state.total : (state is ServiceProviderServicesOrdersLoadedState ? state.total : filteredOrders.length))})",
                ...
                return Padding(
                  ...
                  child: GestureDetector(
                    onTap: () => context.push(
                      ...
                      extra: filteredOrders[index],
                    ),
                    child: isFuelUser
                        ? ServiceProviderOrderCard(
                            order: filteredOrders[index],
                            isLast: filteredOrders.last == filteredOrders[index],
                          )
